...
const Products = () => {
  ...
  return (
    <div className="justify-center  flex   gap-6 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 p-5 ">
      {query.data?.map((product) => (
        <div key={product.id} className="flex flex-col justify-center space-y-3 border-white bg-white shadow-sm hover:shadow-lg px-4 py-2 border rounded-lg overflow-hidden max-auto">
          ...
          <Link to={`/detail/${product.id}`} className="hover:bg-gray  border rounded-md text-center  btn p-2">
            <span>Show more</span>
          </Link>
          <button className="bg-uniq hover:bg-gray hover:border hover:text-black rounded-md text-white btn p-2"
